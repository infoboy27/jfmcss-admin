...
export async function POST(request:Request){const auth=request.headers.get('authorization');if(!process.env.CRON_SECRET||auth!==`Bearer ${process.env.CRON_SECRET}`)return fail("UNAUTHENTICATED", 'Unauthorized', 401);
  ...
  return ok({ok:true,overdue:overdue.length,dueSoon:dueSoon.length,renewals:renewals.length,slaRisk:sla.length});}
