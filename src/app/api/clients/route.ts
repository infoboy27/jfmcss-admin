...
export async function POST(request: Request) {
  try {
    ...
    const b = await request.json(); const name=text(b.name,200); if(!name) return NextResponse.json({error:"Nombre requerido"},{status:400});
    ...
    await audit(user.id,"CREATE","CLIENT",rows[0].id,rows[0]); return NextResponse.json({client:rows[0]},{status:201});
  } catch(error){ return apiError(error); }
}
