...
import { parseBody, clientUpdateSchema } from "@/lib/schema";

export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  try{const user=await requireUser();const {id}=await params;assertClientAccess(user,id);
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  try{const user=await requireUser(['SUPER_ADMIN','ADMIN','FINANCE','PM']);const {id}=await params;const before=(await query(`SELECT * FROM clients WHERE id=$1`,[id])).rows[0];if(!before)return fail("NOT_FOUND", 'No encontrado', 404);const b=await parseBody(request, clientUpdateSchema);
