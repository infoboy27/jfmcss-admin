...
import { apiError, numberValue, optionalText } from "@/lib/http";
...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  try{const user=await requireUser();const {id}=await params;if(user.role==='CLIENT'&&user.clientId!==id) throw Object.assign(new Error('FORBIDDEN'),{status:403});
    ...
    if(!client.rows[0])return NextResponse.json({error:'No encontrado'},{status:404});return NextResponse.json({client:client.rows[0],projects:projects.rows,invoices:invoices.rows,tickets:tickets.rows,assets:assets.rows,contacts:contacts.rows});
  }catch(e){return apiError(e)}}

export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  try{const user=await requireUser(['SUPER_ADMIN','ADMIN','FINANCE','PM']);const {id}=await params;const before=(await query(`SELECT * FROM clients WHERE id=$1`,[id])).rows[0];if(!before)return NextResponse.json({error:'No encontrado'},{status:404});const b=await request.json();
    ...
    await audit(user.id,'UPDATE','CLIENT',id,rows[0],before);return NextResponse.json({client:rows[0]});
  }catch(e){return apiError(e)}}
